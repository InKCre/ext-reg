...
- **Current Truth**: Product design, HLD, targeted PoCs, the exact three-repository implementation plan, and the pre-implementation rehearsal are complete. The executable Registry foundation now exists: Python 3.13/FastAPI/Pydantic contracts, D1/R2 repository and public Worker routes, scoped publisher auth, deterministic CLI, generated language-neutral schemas/OpenAPI, and `@inkcre/extension-runtime`. All local checks, remote CI, package release `v0.1.0`, and exact-main production CD are green. `InKCre/ext-reg` is public; production D1/R2 exist; the public Worker is live at `https://inkcre-extension-registry.lanzhijiang.workers.dev`. The `inkcre` namespace has separate active publisher credentials for core-py and client-web, raw values exist only in the corresponding GitHub Secrets, and a disabled temporary credential proved the public publish → resolve → manifest/file download → canonical/file digest path with `inkcre/blackbox@0.1.0`. Evidence is split between [`work/12-registry-implementation.md`](work/12-registry-implementation.md) and [`work/13-production-delivery.md`](work/13-production-delivery.md).
- **Next Step**: Merge and release Runtime/API `0.1.1` with verified Python 3.12 support, then implement the shared deployment lock plus peer-owned target publication/admission in the fresh worktrees recorded by [`work/14-peer-integration.md`](work/14-peer-integration.md).
