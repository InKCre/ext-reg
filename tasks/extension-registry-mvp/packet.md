...
- **Current Truth**: Product design, HLD, targeted PoCs, the exact three-repository implementation plan, and the pre-implementation rehearsal are complete. The executable Registry foundation now exists: Python 3.13/FastAPI/Pydantic contracts, D1/R2 repository and public Worker routes, scoped publisher auth, deterministic CLI, generated language-neutral schemas/OpenAPI, and `@inkcre/extension-runtime`. All local checks, the real local Worker black box, remote CI, and package release `v0.1.0` are green. `InKCre/ext-reg` is public; production D1/R2 exist; the public Worker is live at `https://inkcre-extension-registry.lanzhijiang.workers.dev`. Production execution exposed and isolated a Cloudflare Pyodide regression after `2026-07-28`; the Worker is currently live from exact source with that verified compatibility date override, while the same pin is staged for an exact-main CD replay. Evidence is split between [`work/12-registry-implementation.md`](work/12-registry-implementation.md) and [`work/13-production-delivery.md`](work/13-production-delivery.md).
- **Next Step**: Commit and replay the verified Python compatibility pin through exact-main CI/CD, seed scoped peer publisher credentials, run the public Registry black box, and then open fresh peer worktrees.
