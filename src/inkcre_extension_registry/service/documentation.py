...
async def discover(
    extension: str, version: str, settings: Settings, *, public: bool = True
) -> ReleaseDocumentation:
...
async def public_snapshot(snapshot_id: str) -> db.DocumentationSnapshot:
    snapshot = (
...
async def publish(
    ...
    async def check(release: db.Release) -> db.DocumentationSet | None:
        ...
        if association is not None and not await association.filter(release=release).exists():
            raise RegistryStateError("documentation scope requires its Distribution association")
        current = (
        ...
        if (condition == "*" and current is not None) or (
            ...
            raise DocumentationPreconditionError("documentation changed; read it before replacing")
    ...
    async def stage() -> None:
        ...
        try:
            for path in paths:
                async with repository.documentation_capacity:
                    key = object_key(bundle.digest, path)
                    stored = await repository.artifacts.head(key)
                    if stored is None:
                        await repository.artifacts.put(
                            key,
                            bundle.files[path],
                            content_type=bundle.manifest[path]["media_type"],
                        )
                    elif stored.size != len(bundle.files[path]):
                        raise RegistryConflictError("documentation staging object size conflict")
    ...
    try:
        ...
        raise RegistryConflictError("documentation snapshot address is already bound") from error
