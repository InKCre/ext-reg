...
Core SDK 尚未提供独立发布产物，因此开发依赖固定 client-web 的 Git revision；该子目录包只提供 manifest 和依赖，不能冒充构建产物。`prepare:sdk` 在临时完整 checkout 中用生产者自己的锁文件构建并打包 SDK，再将产物放入当前 workspace 的 node_modules 私有目录。它只替换当前 Runtime 的依赖链接，不改 pnpm store、生产依赖、源码或锁文件；首次构建需要访问 GitHub 和包仓库。type-check 与 build 共用这一准备步骤，仓库门禁还通过真实 SDK 和本地 HTTP 运行 `check-boundaries.mjs`。正式 SDK 产物可用后，应改为固定产物开发依赖并删除准备脚本，而不是继续维护另一套 SDK 声明。
