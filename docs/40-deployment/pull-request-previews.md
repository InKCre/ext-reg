...
PR 预览使用 `Dockerfile` 构建的 CPython 服务镜像，覆盖目录、扩展详情、Publisher、Simple API 和文件分发。每个同仓库 PR 拥有 Heroku Eco app `inkcre-ext-reg-pr-<number>`、同名私有 R2 桶，以及 Neon 分支 `preview/ext-reg/pr-<number>`。模板不区分预览和生产，环境只提供运行配置。

新数据库分支来自专用空分支 `preview-base`，运行当前提交的完整迁移历史。基础分支不承载业务数据，不随生产导入而改变；不复制生产数据或凭据。预览分支首次配置时更换继承的 `registry_owner` 密码。owner 连接只用于交付中的迁移；迁移创建普通 `registry_app` 角色，控制器设置该分支独立的登录密码，应用的 `DATABASE_URL` 只使用此角色。更新保留现有应用密码；owner 与平台控制凭据不写入 Heroku 配置。更新复用现有分支和桶，保留人工验收数据，仅向前迁移。每次更新将分支到期时间延长七天；到期后数据会丢失，需要长期保留时由运维在 Neon 调整期限。

`registry-preview.yml` 仅接受成功的 `Registry checks`，核验精确 PR head 和同仓库身份。无部署凭据的 build runner 构建镜像，再把镜像交给独立 delivery runner。后者检出可信默认分支控制器，队列后重新确认 PR 仍开放且 head 未改变，再配置资源、迁移和部署。`/livez` 返回镜像构建时写入的提交号，部署探测必须匹配本次提交；app 配置不得覆盖镜像版本，避免旧实例因配置重启而被误认为新版本。执行迁移的短期候选容器只获得所属分支的 owner 连接；运行服务获得普通应用连接和对应 R2 桶的对象读写凭据。Heroku、Neon 和 Cloudflare 控制 token 不进入候选容器。
